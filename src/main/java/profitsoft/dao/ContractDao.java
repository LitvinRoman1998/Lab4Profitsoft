...
public class ContractDao extends InsertedId implements IDao<Contract> {
    ...
    public void create(Contract entity) {
        ...
        clientDao.create(entity.getClient());
        int clientId = clientDao.getLastInsertedId();
        ...
        for (InsuredPerson insuredPerson : entity.getInsuredPeople()) {
            ...
            insuredPeopleIdList.add(insuredPersonDao.getLastInsertedId());
        }
    ...
    public Contract read(long id) {
        ...
        try (Connection connection = ConnectionFactory.getMySQLConnection();
            ...
            if (resultSet.next()) {
                ...
                contract.setClient(clientDao.read(resultSet.getInt(CONTRACT_FK_CLIENT_ID)));
            }
            resultSet.close();
        } catch (SQLException e) {
            System.out.println("Error: " + e.getMessage());
        ...
        for (Integer insuredPersonId : insuredPeopleIdList) {
            insuredPeople.add(insuredPersonDao.read(insuredPersonId));
        }
    ...
    public void update(Contract entity) {
        ...
        try (Connection connection = ConnectionFactory.getMySQLConnection();
            ...
            clientDao.update(entity.getClient());

            for(InsuredPerson insuredPerson:entity.getInsuredPeople()){
                insuredPersonDao.update(insuredPerson);
            }
